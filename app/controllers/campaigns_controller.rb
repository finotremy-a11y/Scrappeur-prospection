class CampaignsController < ApplicationController
  def index
    ...
    Organization.categories.each_key do |cat|
      scope = Organization.where(category: cat)
      @stats[cat] = {
        total:        scope.count,
        with_email:   scope.with_email.count,
        ready:        scope.ready_to_contact.count,
        already_sent: scope.where.not(email_sent_at: nil).count
      }
    end
  ...
  def preview
    category = params[:category].presence
    category = "town_hall" unless Organization.categories.key?(category)

    org = Organization.where(category: category).with_email.first
